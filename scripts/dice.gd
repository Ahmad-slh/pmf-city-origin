...
func roll_dice(p_dice: int = -1) -> void:
	...
	# ------------------------------------------------------
	# لا يرمى النرد ما دامت بطاقة معروضة على الشاشة:
	# سؤال قطاع، أو سؤال معركة، أو بطاقة حدث شارع،
	# أو نافذة اختيار من يجيب في المعركة.
	#
	# الفحص هنا وليس في _on_input_event فقط، حتى يشمل أي
	# استدعاء آخر للدالة مستقبلا وليس ضغطة الفأرة وحدها
	# ------------------------------------------------------
	if board != null and board.is_any_card_open():
		print("لا يمكن رمي النرد أثناء عرض بطاقة")
		return
